...
if (is_user_login()) {
    ...
    $panier = sqlQueryPrepare("SELECT Name, Price, Quantity FROM panier INNER JOIN produit ON panier.Itemid = produit.ID WHERE panier.ClientId = :clientId",
                            ['clientId' => $userId]);
    
    if (count($panier) > 0) {
        for ($i = 0; $i < count($panier); $i++) {
            $amount += $panier[$i]['Quantity'];
        }
    }
}
?>
    ...
    <ul class="header">
        ...
        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
            ...
            <? } else { ?>
                ...
                <a href="/checkout" class="btn btn-primary header-button" style="display: flex; align-items: center; justify-content: center;"><?= $amount ?>
